...
class TravelBooking(Document):
    ...
    def on_submit(self):
        for item in self.booking_package:
            package = frappe.get_doc("Travel Package", item.package)
    ...
    def on_update(self):
       ...
       for item in self.booked_package:
           doc=frappe.get_doc ("Travel Package",item.travel_package)
           doc.available_slots-=item.quantity
           doc.save         
       ...
       if customer_email:
           voucher_pdf=frappe.get_print("Travel Booking",self.name ,print_format="Booking Voucher",as_pdf=True)
           itenary_pdf=frappe.get_print("Travel Booking",self.name ,print_format="Travel Itenary",as_pdf=True)
